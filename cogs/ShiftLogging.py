...
class ShiftLogging(commands.Cog):
    ...
    @duty.command(
        name="time",
        description="Awwows u to check ur shift time, as weww as past data. >w<",
        extras={"category": "Shift Management~"},
        with_app_command=True,
    )
    @is_staff()
    @require_settings()
    @app_commands.describe(member = "Da staff membew to view shifts fow. uwu~", shift_type="Da type of shift to view. uwu~")
    @app_commands.autocomplete(shift_type=shift_type_autocomplete)
    async def duty_time(self, ctx, member: typing.Optional[discord.Member] = None, shift_type: str = "Default uwu~"):
        ...
        if not configItem.get("shift_management", {}).get("enabled", False):
            return await new_failure_embed(ctx, "Nyot Enabled uwu~", "Shift Logging is nyot enabled on dis sewvew.~")
            ...
            if shift_type_value != "all" and shift_type_value not in [st["name"].lower() for st in shift_types]:
                view = CustomSelectMenu(
                    ...
                    [
                        ...
                        discord.SelectOption(label="Aww uwu~", value="all", description="Aww shift types owo~")
                    ],
                )
                msg = await ctx.reply(
                    embed=discord.Embed(title="Incowwect Shift Type owo~",
                                        description="Da shift type pwovided is nyot vawid. owo~",
                                        color=BLANK_COLOR),
                    view=view,
                )
        ...
        if shift_type_value.lower() == "all" or shift_type_value.lower() == "default":
            ...
            if not selected_shift_type:
                return await new_failure_embed(ctx, "Ewwow >w<", "Invawid shift type sewected. uwu~")
        ...
        embed.add_field(
            ...
            value=f"{td_format(datetime.timedelta(seconds=total_seconds))} {'{}*Met Quota*'.format(newline) if met_quota else '{}*Nyot Met Quota*'.format(newline) if met_quota is not None else ''}",
        )
    ...
    @duty.command(
        name="admin",
        description="Awwows fow u to administwate someone else's shift~",
        extras={"category": "Shift Management~"},
    )
    @require_settings()
    @is_admin()
    ...
    async def duty_admin(
        self, ctx, member: discord.Member, type: str = "Default uwu~", force: str = "false"
    ):
        ...
        if shift_types:
            if type.lower() not in [t["name"].lower() for t in shift_types]:
                msg = await ctx.send(
                    embed=discord.Embed(
                        title="Incowwect Shift Type owo~",
                        description="Da shift type pwovided is nyot vawid. owo~",
                        color=BLANK_COLOR,
                    ),
                    view=(
                            ...
                            if not msg:
                                return await ctx.send(
                                    embed=discord.Embed(
                                        ...
                                        description="Dis individuaw does nyot hab access to dis shift type. >w<",
                                        color=BLANK_COLOR,
                                    )
                                )
                            else:
    ...
    @duty.command(
        name="manage",
        description="Manage ur own shift in an easy way! >w<",
        extras={"category": "Shift Management~"},
    )
    @is_staff()
    @require_settings()
    @app_commands.autocomplete(type=shift_type_autocomplete)
    async def duty_manage(self, ctx, *, type: str = "Default uwu~"):
        settings = await self.bot.settings.find_by_id(ctx.guild.id)
        if not settings.get("shift_management", {}).get("enabled", False):
            return await ctx.send(
                embed=discord.Embed(
                        ...
                        if access is False:
                            if not msg:
                                return await ctx.send(
                                    embed=discord.Embed(
                                        title="Access Denied owo~",
                                        description="U do nyot hab access to dis shift type. uwu~",
                                        color=BLANK_COLOR,
                                    )
                                )
                            else:
            ...
            if (on_duty_staff) >= (maximum_staff or 0) and (maximum_staff or 0) != 0:
                return await ctx.send(
                    embed=discord.Embed(
                        ...
                        description="Da maximum amount of staff membews on duty has been reached.",
                        color=BLANK_COLOR,
                    )
                )
    ...
    @duty.command(
        name="active",
        description="Get aww membews of da sewvew cuwwentwy on shift. owo~",
        extras={"category": "Shift Management~"},
    )
    @require_settings()
    ...
    async def duty_active(self, ctx: commands.Context, *, type: str = None):
                ...
                view = CustomSelectMenu(
                    ...
                    + [
                        discord.SelectOption(
                            label="Aww uwu~",
                            value="all",
                            description="Data fwom aww shift types uwu~",
                        )
                    ],
                )
        ...
        for index, staff in enumerate(sorted_staff):
            ...
            if member.id not in added_staff:
                embeds[
                    -1
                ].description += f"\n**{index+1}.** {member.mention} • {td_format(datetime.timedelta(seconds=staff['total_seconds']))}{(' **(Cuwwentwy on bweak: {})**'.format(td_format(datetime.timedelta(seconds=staff['break_seconds'])))) if staff['break_seconds'] > 0 else ''}"
        ...
        if len(embeds) == 1:
            if embeds[0].description == "**Totaw Shifts**":
                try:
                    return await msg.edit(
                        embed=discord.Embed(
                            title="Nyo Active Shifts~",
                            description="Nyo active shifts hab been found in dis sewvew. owo~",
                            color=BLANK_COLOR,
                        ),
                        view=None,
                    )
    ...
    @duty.command(
        name="leaderboard",
        description="Get da totaw time wowked fow da whowe of da staff team.~",
        extras={"category": "Shift Management~"},
        aliases=["lb"],
    )
    @require_settings()
    ...
    async def shift_leaderboard(self, ctx: commands.Context, *, type: str = None):
        ...
        if len(all_staff) == 0:
            return await ctx.send(
                embed=discord.Embed(
                    title="Nyo Shifts >w<",
                    description="Nyo shifts hab been found in dis sewvew. uwu~",
                    color=BLANK_COLOR,
                )
            )
        ...
        embed = discord.Embed(color=BLANK_COLOR, title="Shift Leadewboawd >w<")
        embed.set_author(
            name=f"{ctx.guild.name}",
            icon_url=ctx.guild.icon,
        )
            ...
            if shift_type != 0 and shift_type is not None:
                if not msg:
                    return await ctx.send(
                        embed=discord.Embed(
                            title="Nyo Shifts >w<",
                            description="Nyo shifts hab been found in dis sewvew fow dis Shift Type.",
                            color=BLANK_COLOR,
                        )
                    )
                else:
            ...
            if member:
                ...
                if (
                    ...
                    new_embed = discord.Embed(
                        color=BLANK_COLOR, title="Shift Leadewboawd >w<"
                    )
        ...
        if len(embeds) == 1:
            ...
            if await management_predicate(ctx):
                view = RequestGoogleSpreadsheet(
                    ...
                    scope,
                    combined,
                    config("DUTY_LEADERBOARD_ID uwu~"),
                    total_seconds,
                )
            else:
                view = None
            await ctx.reply(
                embeds=new_embeds,
                file=discord.File(fp=BytesIO(bbytes), filename="shift_leadewboawd.txt"),
                view=view,
            )
        else:
            if await management_predicate(ctx):
            ...
            async def response_func(
                ...
                file = discord.File(
                    fp=BytesIO(bbytes), filename="shift_leadewboawd.txt"
                )
    ...
    @duty.command(
        name="history",
        description="Show aww past shifts of usew by shift type >w<",
        extras={"category": "Shift Management~"},
        aliases=["h"],
    )
    @require_settings()
    ...
    async def duty_shifts(
        self, ctx: commands.Context, user: discord.User, type: str = "Default uwu~"
    ):
        if self.bot.shift_management_disabled is True:
            return await new_failure_embed(
                ctx,
        ...
        if not shift_type_item:
            if type.lower() == "default":
                shift_type_item = {"name": "Default uwu~"}
            else:
                return await ctx.send(
                    embed=discord.Embed(
                        title="Incowwect Shift Type owo~",
        ...
        if not shifts:
            return await ctx.send(
                embed=discord.Embed(
                    title="Nyo Shifts >w<",
                    description="Nyo shifts hab been found fow dis usew. >w<",
                    color=BLANK_COLOR,
                )
            )
